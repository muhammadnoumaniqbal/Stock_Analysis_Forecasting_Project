...
with col3:
    end_date = st.datetime_input("Choose End Date", datetime.date(today.year, today.month, today.day))
...
col1.metric(
    "Daily Changes",
    str(round(data['Close'].iloc[-1], 2)),
    str(round(daily_changes, 2))
)
...
# 7 columns for 7 buttons (instead of 12)
...
col1, col2, col3 = st.columns([2, 2, 4]) # zyada space for first two selectboxes
with col1:
...
if num_period == '':
    ...
    if chart_type == 'Candle' and indicators == 'RSI':
        st.plotly_chart(candlestick(data1, '1y', num_period), use_container_width=True)
    ...
    if chart_type == 'Line' and indicators == 'RSI':
        st.plotly_chart(close_chart(data1, '1y', num_period), use_container_width=True)
        st.plotly_chart(RSI(new_df1, num_period), use_container_width=True)

    if chart_type == 'Line' and indicators == 'Moving Average':
        st.plotly_chart(Moving_average(data1, '1y', num_period), use_container_width=True)
